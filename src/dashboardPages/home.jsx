    ...
    tooltip: {
      ...
      y: {
        formatter: function (val, { seriesIndex }) {
          return seriesIndex === 0 ? `Sales: $${val.toLocaleString()}` : `Orders: ${val.toLocaleString()}`
        }
      }
    }
  };
    ...
    <div className="p-6 bg-gray-50">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        {cardData.map((card, index) => (
          ...
          >
            <div className={`${card.color} text-white p-4 rounded-lg mr-4 flex items-center justify-center shadow-md`}>
              <span className="text-xl">{card.icon}</span>
            </div>
            <div>
              <p className="text-gray-500 text-sm">{card.title}</p>
              <p className="text-2xl font-bold text-gray-800">{card.value}</p>
              <p className="text-xs text-green-500 font-medium">{card.increase} from last month</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="bg-[#FFFFFF] rounded-xl shadow-md p-4 mb-8">
        ...
        <div className="h-96">
          <ReactApexChart 
            options={chartType === 'line' ? lineChartOptions : barChartOptions} 
            series={chartSeries} 
            type={chartType} 
            height="100%" 
          />
